...
<body>
	<div class="wrapper">
		<?php
			...
			// $cardArray not needed. Kept for reference.
			$cardArray = ["1s", "2s", "3s", "4s", "5s", "6s", "7s", "8s", "9s", "10s", "11s", "12s", "13s",
			...
			for ($a=0; $a<4; $a++) {
				...
				if ($a==3) {$suit="h";}
  				...
  				for ($i=0; $i<13; $i++) {
  					$image_string='"../cards/' . ($i+1) . $suit . '.png"';
  				    echo '<div id="card' . $card . '"><img src=' . $image_string . ' alt="card graphic"/></div>';
  				}
  				echo '</div>';
  			}	
		?>
				...
				while (cardIsDuplicate==true) {
					...
					for (a=0; a<52; a++) {
						if (cardArray[a] == rank+suit) {
							cardIsDuplicate = true;
							console.log (cardArray[a] + "is a duplicate.");
						}
					}
					if (cardIsDuplicate==false) {
						cardArray[i]=rank+suit;
						console.log(cardArray[a]);
